...
class UploadService extends Service {
  ...
  async create(stream) {
    ...
    // 插入数据库
    return ctx.model.Upload.create({
      name: filename,
      extname: _extname,
      mimeType: _mimeType,
      size: _size,
  ...
  async index(query) {
    ...
    if (pageSize) {
      ...
      result = await ctx.model.Upload.findAndCountAll({
        // offet去掉前多少个数据
        offset: _offset,
        // limit每页数据数量
        limit: pageSize,
      ...
      result = await ctx.model.Upload.findAll();
    }
    return result;
    ...
    //   if (search) {
    //     if (kind) {
    //       res = await ctx.model.Upload.findOne({ filename: { $regex: search }, extname: { $in: attachmentKind[`${kind}`] } }).skip(skip).limit(Number(pageSize))
    //         .sort({ createdAt: -1 })
    //         .exec();
    //     } else {
    //       res = await ctx.model.Upload.findOne({ filename: { $regex: search } }).skip(skip).limit(Number(pageSize))
    //         .sort({ createdAt: -1 })
    //         .exec();
    //     }
    //     totals = res.length;
    //   } else {
    //     if (kind) {
    //       res = await ctx.model.Upload.findOne({ extname: { $in: attachmentKind[`${kind}`] } }).skip(skip).limit(Number(pageSize))
    //         .sort({ createdAt: -1 })
    //         .exec();
    //       totals = await ctx.model.Upload.count({ extname: { $in: attachmentKind[`${kind}`] } }).exec();
    //     } else {
    //       res = await ctx.model.Upload.findOne({}).skip(skip).limit(Number(pageSize))
    //         .sort({ createdAt: -1 })
    //         .exec();
    ...
    //   if (search) {
    //     if (kind) {
    //       res = await ctx.model.Upload.findOne({ filename: { $regex: search }, extname: { $in: attachmentKind[`${kind}`] } }).sort({ createdAt: -1 }).exec();
    //     } else {
    //       res = await ctx.model.Upload.findOne({ filename: { $regex: search } }).sort({ createdAt: -1 }).exec();
    //     }
    //     totals = res.length;
    //   } else {
    //     if (kind) {
    //       res = await ctx.model.Upload.findOne({ extname: { $in: attachmentKind[`${kind}`] } }).sort({ createdAt: -1 }).exec();
    //       totals = await ctx.model.Upload.count({ extname: { $in: attachmentKind[`${kind}`] } }).exec();
    //     } else {
    //       console.log(ctx.model.Upload);
    //       res = await ctx.model.Upload.findAll();
    //       // res = await ctx.model.Upload.findOne({}).sort({ createdAt: -1 }).exec();
    //       // totals = await ctx.model.Upload.count({}).exec();
    //     }
    //   }
    // }
  ...
  async destroy(id) {
    ...
    // 数据库删除文件
    return ctx.model.Upload.findByIdAndRemove(id);
  }

  // 查询单个附件
  async show(id) {
    const { ctx } = this;
    const attachment = await ctx.model.Upload.findByPk(id);
    if (!attachment) {
      ctx.throw(404, 'attachment not found');
    }
    return attachment;
