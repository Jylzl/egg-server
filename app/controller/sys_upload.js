...
class SysUploadController extends Controller {
  ...
  async update() {
    ...
    const attachment = await service.sysUpload.updatePre(id);
    // 获取用户上传的替换文件
    const stream = await ctx.getFileStream();
    ...
    await service.sysUpload.update(id, attachment);
    // 设置响应内容和响应状态码
    ctx.helper.success({
      ctx,
    });
  ...
  async url() {
    ...
    try {
      ...
      res = await service.sysUpload.create(attachment);
    } catch (err) {
      throw err;
    }
    // 设置响应内容和响应状态码
    ...
    while ((part = await parts()) != null) {
      if (part.length) {
        ...
        try {
          ...
          // 调用Service
          await service.sysUpload.create(attachment);
        } catch (err) {
          // 必须将上传的文件流消费掉，要不然浏览器响应会卡死
  ...
  async destroy() {
    ...
    // 调用 Service 进行业务处理
    await service.sysUpload.destroy(id);
    // 设置响应内容和响应状态码
    ctx.helper.success({
      ctx,
    });
  ...
  async extra() {
    ...
    await service.sysUpload.extra(id, payload);
    // 设置响应内容和响应状态码
    ctx.helper.success({
      ctx,
    });
  ...
  async show() {
    ...
    // 调用 Service 进行业务处理
    const res = await service.sysUpload.show(id);
    // 设置响应内容和响应状态码
    ctx.helper.success({
      ctx,
      res,
  ...
  async removes() {
    ...
    for (const attachment of payload) {
      // 调用 Service 进行业务处理
      await service.sysUpload.destroy(attachment);
    }
    // 设置响应内容和响应状态码
    ctx.helper.success({
      ctx,
